...
class Solution {
    ...
    private func secondStep() {
        for (face, number) in zip(self.cube.faces[0...3], [2, 20, 18, 0]) {
            if isNumberInUp(number: number) {
                moveNumber(face: face, number: number)
                moveNuberToDown(face: face, number: number)
            } else {
                let faceOnePifPaf = moveNuberFromDown(number: number)!
                pifPaf(fase: faceOnePifPaf)
                moveNumber(face: face, number: number)
                moveNuberToDown(face: face, number: number)
            }
        }
    ...
    // Комбинация пиф-паф (относительно текущей грани R U R' U')
    private func pifPaf(fase: Face) {
    ...
    // Проверка находится ли номер в верхнем слое (где Up)
    private func isNumberInUp(number: Int) -> Bool {
        return
    ...
    private func moveNumber(face: Face, number: Int) {
        let coordinate: (Int, Int, Int)
        switch face.flip {
        case .L:
            coordinate = (0, 2, 2)
